    ...
    public class SeparationRel : ContextBehavior
    {
        ...
        protected override float ScoreAngle(float angle, Vector2 position, Vector2 momentum)
        {
            ...
            if (fleet != null)
            {
                ...
                {
                    var fdist = Vector2.Distance(other.Center + other.Velocity * LookAheadMS, position);
                    var dist = fdist + 0.0f;
                    if (dist < ActiveRange * 2.0f)
                    {
                        foreach (var ship in fleet.Ships)
                        {
                            ...
                            {
                                dist = MathF.Min(Vector2.Distance(ship2.Position + other.Velocity * LookAheadMS, position + ship.Position - fleet.Center), dist);
                            }
                        }
                        if (dist < ActiveRange)
                        {
                            accumulator -= Vector2.Dot(other.Center + other.Velocity * LookAheadMS - position, new Vector2(MathF.Cos(angle), MathF.Sin(angle))) / fdist / dist;
                        }
                    }
                }
            }
